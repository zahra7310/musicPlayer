...
<template>
  <div v-if="showPlayList">
    <podcasts
     :playCurrentPodcast="playCurrentPodcast"  
    />
  </div>
  <div v-else>
      ...
      <main class="grid min-h-screen">
        <div>
          <section class="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <div
              v-for="(podcast,index) in $store.state.podcasts"
              :key="podcast.id"
...
export default {
  data() {
    ...
    let playCurrentPodcast=[]
    return {
      showPlayList,playCurrentPodcast
    };
  },
  methods: {
    goToPlayList(index) {
      this.showPlayList = true;
      this.playCurrentPodcast = this.$store.state.podcasts[index];
    },
  },
};
</script>
